| | |
| :--- | :--- |
| **Mata Kuliah** | : Desain dan Pemrograman Web |
| **Program Studi** | : D4 – Teknik Informatika |
| **Semester** | : 3 |

---

| | |
| :--- | :--- |
| **Kelas** | : TI-2D |
| **NIM** | : 254107020019 |
| **Nama** | : M. Javier Thufail |
| **Jobsheet Ke-** | : 2 |

# LAPORAN JOBSHEET

### **1. Keterangan Kode HTML**
- Tidak ada perubahan kode — halaman HTML tetap sama persis dengan Jobsheet 3.
- Tambah docs/wireframe.md: wireframe teks + user flow untuk fitur yang belum dibangun (Login, Dashboard Petugas, Peminjaman, Pengembalian, Riwayat).

### **1. Keterangan Kode CSS**
- Sebenarnya untuk css juga tidak ada yang berubah tetapi saya ingin mengubah warna dari SIMPUS-MINI nya agar lebih menarik.
```bash
/* ===== Reset & Base ===== */
* {
    box-sizing: border-box;
    margin: 0;
    padding: 0;
}

body {
    font-family: "Segoe UI", Arial, sans-serif;
    color: #334155;
    background-color: #eef2f3;
    line-height: 1.5;
}

a {
    color: #0f766e;
    text-decoration: none;
}

a:hover {
    text-decoration: underline;
}

/* ===== Header & Navbar (Flexbox) ===== */
header {
    background-color: #183b3a;
    color: #fff;
    padding: 1rem 1.5rem;
    display: flex;
    align-items: center;
    justify-content: space-between;
    flex-wrap: wrap;
}

header h1 {
    font-size: 1.4rem;
    color: #f1f5f4;
}

header nav ul {
    list-style: none;
    display: flex;
    gap: 1.25rem;
}

header nav a {
    color: #d5e5e2;
    font-weight: 500;
}

header nav a:hover {
    color: #8fd3c7;
}

/* ===== Main Layout ===== */
main {
    max-width: 1000px;
    margin: 2rem auto;
    padding: 0 1.5rem;
}

section {
    background-color: #ffffff;
    border-radius: 8px;
    padding: 1.5rem;
    margin-bottom: 1.5rem;
    box-shadow: 0 2px 6px rgba(24, 59, 58, 0.10);
    border: 1px solid #dce5e4;
}

section h2 {
    margin-bottom: 1rem;
    color: #245e5a;
}

/* ===== Kartu Statistik (CSS Grid) ===== */
main section:nth-of-type(2) {
    display: grid;
    grid-template-columns: repeat(4, 1fr);
    gap: 1rem;
}

main section:nth-of-type(2) article {
    background-color: #e5f0ee;
    border-radius: 8px;
    padding: 1.25rem;
    text-align: center;
    flex: 1;
    border: 1px solid #cbdedb;
}

main section:nth-of-type(2) h2 {
    grid-column: 1 / -1;
    width: 100%;
    margin-bottom: 0.5rem;
}

main section:nth-of-type(2) article h3 {
    font-size: 0.95rem;
    color: #526866;
    margin-bottom: 0.5rem;
    grid-column: 1 / -1;
}

main section:nth-of-type(2) article p {
    font-size: 1.8rem;
    font-weight: 700;
    color: #176b63;
}

/* ===== Tabel ===== */
table {
    width: 100%;
    border-collapse: collapse;
}

th,
td {
    text-align: left;
    padding: 0.65rem 0.75rem;
    border-bottom: 1px solid #dce5e4;
}

thead {
    background-color: #245e5a;
    color: #fff;
}

tbody tr:nth-child(even) {
    background-color: #f3f6f6;
}

tbody tr:hover {
    background-color: #e2eeec;
}

td button {
    padding: 0.35rem 0.6rem;
    margin-right: 0.35rem;
    border: none;
    border-radius: 4px;
    cursor: pointer;
    font-size: 0.85rem;
}

td button:first-of-type {
    background-color: #356bb3;
    color: #fff;
}

td button:nth-of-type(2) {
    background-color: #b7791f;
    color: #fff;
}

td button:last-of-type {
    background-color: #b8324b;
    color: #fff;
}

/* ===== Form ===== */
form p {
    margin-bottom: 1rem;
}

form label {
    display: block;
    margin-bottom: 0.35rem;
    font-weight: 600;
    color: #334d4a;
}

form input, 
form select {
    width: 100%;
    max-width: 400px;
    padding: 0.55rem 0.7rem;
    background-color: #f8faf9;
    border: 1px solid #c4d2d0;
    border-radius: 4px;
    font-size: 1rem;
    color: #334155;
}

form button[type="submit"] {
    background-color: #245e5a;
    color: #fff;
    border: none;
    padding: 0.6rem 1.5rem;
    border-radius: 4px;
    font-size: 1rem;
    cursor: pointer;
}

form button[type="submit"]:hover {
    background-color: #183f3c;
}

/* ===== Footer ===== */
footer {
    text-align: center;
    padding: 1.25rem;
    color: #687876;
    font-size: 0.9rem;
}


/* ===== Elemen Responsif (Table & Code Block) ===== */
.table-responsive,
.code-responsive,
pre {
    overflow-x: auto;
    max-width: 100%;
    display: block;
}

pre code {
    white-space: pre; 
}

/* ===== Hamburger Menu (checkbox hack) ===== */
.nav-toggle {
    display: none;
}

.nav-toggle-label {
    display: none;
    font-size: 1.6rem;
    color: #fff;
    cursor: pointer;
}

/* ===== Responsive Breakpoints ===== */

@media (min-width: 1400px) {
    main {
        max-width: 1000px;
    }
}

/* Tablet ke bawah */
@media (max-width: 900px) {
    main section:nth-of-type(2) {
        grid-template-columns: repeat(2, 1fr);
    }
}

/* Mobile */
@media (max-width: 480px) {
    header {
        position: relative;
    }

    .nav-toggle-label {
        display: block;
    }

    header nav {
        display: none;
        width: 100%;
        order: 3;
        margin-top: 1rem;
    }

    .nav-toggle:checked ~ nav {
        display: block;
    }

    header nav ul {
        flex-direction: column;
        gap: 0.75rem;
    }

    main section:nth-of-type(2) {
        grid-template-columns: 1fr;
    }

    form input,
    form select {
        max-width: 100%;
    }
}
```  
**Outputnya:**  
![alt text](img/CSS.png)